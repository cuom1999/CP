...
void compute(int l, int r, int opt_l, int opt_r){
	...
	d[mid]=1e18;
	int opt_mid=0;
	//cout<<mid<<" "<<opt_l<<" "<<opt_r<<endl;

	for (int i=opt_l; i<mid-1 && i<=opt_r; i++){
		ll M=d[i]+getMax(i+1,mid)-getMin(i+1,mid);
	...
	compute(l,mid-1,opt_l, opt_mid);
	compute(mid+1,r,opt_mid,opt_r);
}
...
{IN;
	...
	FOR (i,2,n){
        d[i]=1e18;
        FOR (j,0,i-2){
            d[i]=min(d[i],d[j]+getMax(j+1,i)-getMin(j+1,i));
        }
    }
    cout<<d[n]<<endl;
