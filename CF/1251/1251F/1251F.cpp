...
int combi(int n, int k) {
    if (n < k || k < 0) return 0;
    return fact[n] * 1ll * inv[n - k] % MOD * inv[k] % MOD;
}
...
void solve(int redHeight) {
    ...
    Polynomial A = {1, 2};
    Polynomial B = {1, 1};
    A = pw(A, num1, MOD, 2 * num1);
    B = pw(B, 2 * num2, MOD, 4 * num2);
    trim(A), trim(B);
    Polynomial C = multiply(A, B, MOD);
    for (int i = 0; i < C.size(); i++) {
        int peri = 2 * (redHeight + i + 1);
        res[peri] = (res[peri] + C[i]) % MOD;
    }
}
