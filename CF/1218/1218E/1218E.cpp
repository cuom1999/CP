...
namespace Poly {
    ...
    Polynomial multiplyBrute(Polynomial& a, Polynomial &b, int mod, int maxDeg = -1) {
    ...
    Polynomial multiply(Polynomial &a, Polynomial& b, int mod, int maxDeg = -1) {
    ...
    Polynomial inverse(Polynomial &a, int mod, int maxDeg) {
        assert(a[0]);
        int n = 1;
        Polynomial R, V, G;
        a[0] = (a[0] % mod + mod) % mod;
    ...
    Polynomial sqrt(Polynomial& a, int mod, int maxDeg) {
        assert(a[0] == 0 || a[0] == 1);
    ...
    Polynomial derivative(Polynomial& a, int mod) {
        Polynomial res;
    ...
    Polynomial integrate(Polynomial& a, int mod) {
        Polynomial res;
        res.push_back(0);
    ...
    Polynomial ln(Polynomial& a, int mod, int maxDeg) {
    ...
    Polynomial exp(Polynomial& a, int mod, int maxDeg) {
        assert(a[0] == 0);
    ...
    Polynomial pwBySquare(Polynomial& a, long long n, int mod, int maxDeg) {
        if (n == 0) return {1};
    ...
    Polynomial pwByExp(Polynomial& a, long long n, int mod, int maxDeg) {
        int minCoef = -1;
        for (int i = 0; i < a.size(); i++) {
            if (a[i]) {
                minCoef = i;
    ...
    Polynomial pw(Polynomial& a, long long n, int mod, int maxDeg) {
...
struct Solver{
    ...
    Polynomial solve(int l, int r) {
        ...
        int mid = (l + r) / 2;
        Polynomial polyL = solve(l, mid);
        Polynomial polyR = solve(mid + 1, r);
        Polynomial res = multiply(polyL, polyR, MOD);
        return res;
    }
};
...
{IN;
    ...
    FOR (z, 1, q) {
        ...
        if (type == 1) {
            ...
            cin >> val >> pos >> d;
            FOR (i, 1, n) {
                solver.b[i] = (val - a[i] + MOD) % MOD;
                if (i == pos) {
                    solver.b[i] = (val - d % MOD + MOD) % MOD;
                }
            }
        }
        else {
            ...
            FOR (i, 1, n) {
                solver.b[i] = (val - a[i] + MOD) % MOD;
                if (i >= l && i <= r) {
                    solver.b[i] = (solver.b[i] - d % MOD + MOD) % MOD;
                }
            }   
        }
        Polynomial res = solver.solve(1, n);
        cout << res[n - k] << "\n";
    }
