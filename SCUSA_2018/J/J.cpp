...
{IN;
    ...
    MinCostFlow<int, int> mcmf(n + 3);

    mcmf.addEdge(source1, source, numPeople, 0);
    ...
    FOR (i, 1, m) {
        ...
        x += 2;
        y += 2;
        if (x != sink && y != source) mcmf.addEdge(x, y, 1, 1);
        if (y != sink && x != source) mcmf.addEdge(y, x, 1, 1);
    }

    auto res = mcmf.minCostMaxFlow(source1, sink);

    if (res.first == numPeople) {
        cout << res.second << endl;
    }
    else {
        cout << numPeople - res.first << " people left behind\n";
    }
