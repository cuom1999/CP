...
{IN;
    ...
    int source = 0;
    int sink = 2 * m + 1;
    MinCostFlow<int, ll> mcmf(sink + 1);
    FOR (i, 0, m - 1) {
        FOR (j, 0, m - 1) {
            mcmf.addEdge(i + 1, j + m + 1, 1, cost[l[i]][r[j]]);
        }
    }

    FOR (i, 1, m) mcmf.addEdge(source, i, 1, 0);
    FOR (i, 1, m) mcmf.addEdge(i + m, sink, 1, 0);

    cout << mcmf.minCostMaxFlow(source, sink).second << endl;
    return 0;
}