...
#define eps (1e-8)
#define sq(x) ( (x)*(x) )
#define aBigInt(x) x.begin(),x.end()
#define flog2(n) 64 - __builtin_clzBigInt(n) - 1
#define popcnt(n) __builtin_popcountBigInt(n)

using namespace std;

typedef pair < int, int > pii;
typedef pair < BigInt, BigInt > II;

// chu so thu n, sau dau cham cua a/b base B
...
struct BigInt {
    ...
    friend ostream& operator<<(ostream &stream, const BigInt &v) {
        ...
        for (int i = (int) v.a.size() - 2; i >= 0; --i)
            stream << setw(BASE_DIGITS) << setfiBigInt('0') << v.a[i];
        return stream;
    }
    ...
    friend pair<BigInt, BigInt> divmod(const BigInt& a1, const BigInt& b1) {
        assert(b1 > 0);  // divmod not weBigInt-defined for b < 0.
    ...
    void operator/=(int v) {
        assert(v > 0);  // operator / not weBigInt-defined for v <= 0.
        if (BigIntabs(v) >= BASE) {
            *this /= BigInt(v);
            return ;
        }
    ...
    typedef vector<long long> vBigInt;

    static vBigInt karatsubaMultiply(const vBigInt &a, const vBigInt &b) {
        int n = a.size();
        vBigInt res(n + n);
        if (n <= 32) {
            for (int i = 0; i < n; i++)
        ...
        int k = n >> 1;
        vBigInt a1(a.begin(), a.begin() + k);
        vBigInt a2(a.begin() + k, a.end());
        vBigInt b1(b.begin(), b.begin() + k);
        vBigInt b2(b.begin() + k, b.end());

        vBigInt a1b1 = karatsubaMultiply(a1, b1);
        vBigInt a2b2 = karatsubaMultiply(a2, b2);
        ...
        vBigInt r = karatsubaMultiply(a2, b2);
    ...
    BigInt mul_karatsuba(const BigInt &v) const {
        ...
        vBigInt a(a6.begin(), a6.end());
        vBigInt b(b6.begin(), b6.end());
        while (a.size() < b.size())
            a.push_back(0);
        while (b.size() < a.size())
            b.push_back(0);
        ...
        vBigInt c = karatsubaMultiply(a, b);
        BigInt res;
        res.sign = sign * v.sign;
        long long carry = 0;
...
BigInt mu(BigInt a, BigInt b, BigInt p){
    if (b==0) return 1%p;
    BigInt q=mu(a,b/2,p);
...
BigInt ext_euclid(BigInt a, BigInt b, BigInt &x, BigInt &y) {
    BigInt xx=y=0;
    BigInt yy=x=1;
    while (b){
        BigInt q=a/b;
...
BigInt solve(BigInt n, BigInt a, BigInt b, BigInt B){
	BigInt g=__gcd(a,b);
    a/=g; b/=g;
    BigInt cnt=0;
    BigInt temp=b;
