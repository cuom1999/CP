...
{IN;
    ...
    sort(all(classes), [&](const Class& a, const Class& b) {
    ...
    int sink = n + 1;
    int source1 = n + 2;
    MinCostFlow<int, ll> mcmf(source1 + 1);

    mcmf.addEdge(source1, source, k, 0);

    FOR (i, 1, n) {
        if (classes[i - 1].name.back() == '2') {
            mcmf.addEdge(i - 1, i, 2, classes[i - 1].cost - classes[i - 2].cost);
            mcmf.addEdge(i, sink, 2, 0);
        }
        else {
            mcmf.addEdge(source, i, 2, 2 * classes[i - 1].cost);
            mcmf.addEdge(i, sink, 1, 0);
        }
    }

    cout << mcmf.minCostMaxFlow(source1, sink).second << endl;
    return 0;
}